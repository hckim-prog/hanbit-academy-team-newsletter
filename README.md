...
여러 업무보고를 합칠 때는 완전히 같거나 사실상 같은 문장을 한 번만 남깁니다. 성과·기회는 `반짝 소식`, 진행 업무는 `집중 모드`, 약한 신호·리스크는 `체크 포인트`, 향후 계획은 `다음 2주`에 우선 배치합니다. 한입 요약은 성과·핵심 업무·다음 2주의 실제 항목 가운데 구체성이 높은 내용을 골라 만들며, 요약할 업무가 없으면 해당 섹션을 표시하지 않습니다.
